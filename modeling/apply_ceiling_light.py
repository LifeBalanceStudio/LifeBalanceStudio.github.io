"""기존 GLB의 모든 부품 데이터를 보존하면서 천장등만 추가한다."""
...
OUTPUT = ROOT / "output"
BASE = ROOT / "revisions" / "ceiling-light-before-2026-09-13"
PART = OUTPUT / "ceiling-light.glb"
...
current, current_binary = read_glb(OUTPUT / "room.glb")
assert not any(node.get("name") == "천장등" for node in current["nodes"]), "이미 천장등이 추가된 모델입니다."
document, original_binary = read_glb(BASE / "room.glb")
assert current == document and current_binary == original_binary, "기준 보존 후 다른 모델 변경이 있어 중단합니다."
original = copy.deepcopy(document)

bpy.ops.wm.open_mainfile(filepath=str(OUTPUT / "room.blend"))
lamp = add_ceiling_light()
bpy.ops.object.select_all(action="DESELECT")
lamp.select_set(True)
for child in lamp.children:
    if child.type == "MESH":
...
assert not added.get("textures") and not added.get("images") and not added.get("skins") and not added.get("animations")
...
(OUTPUT / "ceiling-light-report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
